...
export default function HomeownersPage() {
  ...
  useEffect(() => {
    ...
    const routeIsValid = properties.some(
      (property) => String(property.id) === String(homeownerId),
    )

    if (!routeIsValid) {
      navigate(`/homeowners/${properties[0].id}`, { replace: true })
    }
  }, [homeownerId, loading, navigate, properties])
  ...
  }, [directoryEntries, search, statusFilter])
  ...
  const history = useMemo(() => {
    const regular = selectedPayments.map((payment) => ({
      ...
      remaining: Number(payment.remaining_balance) || 0,
    ...
    const services = selectedServices.map((transaction) => ({
      ...
      status:
        normalize(transaction.payment_status) === 'partial'
          ? 'Partial'
          : 'Completed',
    ...
    <div className="homeowners-page">
      <header className="homeowners-header">
        <div>
          ...
          <h1>Homeowners</h1>
          <p>View homeowner profiles, properties, balances, and complete payment history.</p>
        </div>
        <button
          type="button"
      ...
      <div className="homeowners-workspace">
        <aside className="homeowners-directory" aria-label="Homeowner directory">
          <div className="homeowners-directory-heading">
            <div>
              <h2>Directory</h2>
          ...
          <input
            ...
            placeholder="Name, block, lot, phone, or email..."
            value={search}
          ...
          <div className="homeowners-directory-list">
            {loading ? (
              ...
              filteredDirectory.map(({ property, status }) => (
                <button
                  ...
                  onClick={() => navigate(`/homeowners/${property.id}`)}
                >
        ...
        <main className="homeowner-profile-panel">
          {loading ? (
            ...
            <div className="homeowner-profile-empty">
              <Users size={34} />
              <h2>No homeowner selected</h2>
              <p>Add a homeowner in Contact Manager to create a profile.</p>
            </div>
          ) : (
            <>
