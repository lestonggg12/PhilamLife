  ...
  try {
    ...
    if (
      ...
      return json(
        {
          error:
            "Only an active administrator can manage users.",
        },
        403,
      );
    }
    ...
    if (action === "create") {
      ...
      if (createError || !created.user) {
        throw (
          createError ??
          new Error("Account was not created.")
        );
      }
    ...
    if (!targetId) {
      return json(
        {
          error:
            "A user account must be selected.",
        },
        400,
      );
    }
    ...
    if (action === "reset-password") {
      ...
      await writeLog(
        ...
        targetName,
        "RESET_PASSWORD",
        `Sent a password-reset email to ${target.email}.`,
      );
